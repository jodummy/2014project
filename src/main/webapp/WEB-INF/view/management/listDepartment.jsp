...
// 	function detailDepart(code) {
// 		var str = code.split(' ');
// 		alert(str[0]);
// 		alert(str[1]);
// 		var obj = document.forms[0];
// 		obj.action = "./detailDepartment.do?dept_no=" + str[0] + "&dept_top=" +str[1];
// 		obj.method = "post";
// 		obj.submit();
// 	}
	...
	function detailDepart(code) {
		...
		alert(str[0]);
		alert(str[1]);
		window.open( "./detailDepartment.do?dept_no=" + str[0] + "&dept_top=" +str[1], "선택",
						"scrollbars=yes,toolbar=yes,resizable=yes,width=450,height=500,right=150,top=0");
	}
	
	function insertDepartment() {
		window.open( "./insertDepartment.do", "선택",
		"scrollbars=yes,toolbar=yes,resizable=yes,width=450,height=500,right=150,top=0");
	}
</script>
<body>
			...
			<div style="padding-left: 50px;">
				...
				<form action="#" method="post" id='frmPaging'>
					<table class="table table-bordered table-hover">
						...
						<c:forEach items="${memlist}" var="nn">
							<tr>
								<th><input type="checkbox" onclick="checkAll(this.checked)"></th>
								<th>${nn.dept_no}</th>
								<th>${nn.dept_name}</th>
								...
								<th><input type="button" value="조회"
									onclick="detailDepart('${nn.dept_no} ${nn.dept_top}')" 
				...
				<input type="button" value="팀생성" onclick="insertDepartment()" class='btn btn-sm btn-warning'>
			</div>
		</div>
	</div>
