...
function calendarSchduleAdd() {
	...
	if(summary.trim() == '' || summary.trim().length == 0) {
		swal('제목','입력해주세요');
		return false;
	}else if($('#startTime').val() == null) {
		swal('시작시간','입력해주세요');
		return false;
	}else if($('#endTime').val() == null) {
		swal('종료시간','입력해주세요');
		return false;		
	}else if(new Date(0,0,0,$('#endTime').val().split(":")[0],$('#endTime').val().split(":")[1]).getTime() - new Date(0,0,0,$('#startTime').val().split(":")[0],$('#startTime').val().split(":")[1]).getTime() < 0) {
	...
	$.ajax({
		url: './meetingRoomEventAdd.do',
		type: 'post',
		async: false,
		data : $('#frmSchdule').serialize()+'&br_no='+$('#br_no').val(),
...
function modifyEvent() {
	...
	$.ajax({
		url: './meetingRoomEventModify.do',
		type: 'post',
		async: false,
		data: $('#frmSchduleModify').serialize()+'&br_no='+$('#br_no').val(),
		success: function(msg) {
			if(msg.isc) {
				swal('수정', '성공하였습니다');
			}else {
...
function removeEventOne(eventId) {
	$.ajax({
		url: './meetingRoomEventRemoveOne.do',
		type: 'post',
		async: false,
		data : {
			"eventId" : eventId,
			...
			for(var j=0; j<data.cnt; j++) {
				if(i<startIndex) {
					if(monthDay[i] == data.start[j].getDate() && months[0] == data.start[j].getMonth()+1) {
						if(months[0] == 12 && year-1 == data.start[j].getFullYear()) {
					...
					if(data.start[j].getHours() != data.end[j].getHours() || (data.start[j].getMinutes() < 30 && data.end[j].getMinutes() >= 30)) {
						if(monthDay[i] == data.end[j].getDate() && months[0] == data.end[j].getMonth()+1) {
							if(months[0] == 12 && year-1 == data.end[j].getFullYear()) {
					...
					if(monthDay[i] == data.start[j].getDate() && months[2] == data.start[j].getMonth()+1) {
						if(months[2] == 1 && year+1 == data.start[j].getFullYear()) {
							if(data.start[j].getMinutes() < 30) {
								$('.'+(data.start[j].getHours()*2+1)).eq(i%7).html(eventTagFormat(getTime(data.start[j]), data.title[j], data.eventId[j], data.description[j]));
							}else {
								$('.'+(data.start[j].getHours()*2+2)).eq(i%7).html(eventTagFormat(getTime(data.start[j]), data.title[j], data.eventId[j], data.description[j]));
					...
					if(data.start[j].getHours() != data.end[j].getHours() || (data.start[j].getMinutes() < 30 && data.end[j].getMinutes() >= 30)) {
						if(monthDay[i] == data.end[j].getDate() && months[2] == data.end[j].getMonth()+1) {
							if(months[2] == 1 && year+1 == data.end[j].getFullYear()) {
								if(data.end[j].getMinutes() < 30) {
									$('.'+(data.end[j].getHours()*2+1)).eq(i%7).html(eventTagFormatEnd(getTime(data.end[j]), data.title[j], data.eventId[j], data.description[j]));
								}else {
									$('.'+(data.end[j].getHours()*2+2)).eq(i%7).html(eventTagFormatEnd(getTime(data.end[j]), data.title[j], data.eventId[j], data.description[j]));
